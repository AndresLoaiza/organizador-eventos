...
export async function urlFirmada(storageKey, codigo = leerCodigo(), segundos = 300) {
  const { data, error } = await db(codigo).storage.from(BUCKET).createSignedUrl(storageKey, segundos);
  if (error) throw new Error(error.message);
