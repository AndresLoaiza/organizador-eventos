...
import { armarPanorama, hoyMedellin } from './panorama.mjs';
...
export const CLAVE_FESTIVAL = 'canovaccio.festival';

export function leerFestival() {
  try { return localStorage.getItem(CLAVE_FESTIVAL) ?? ''; } catch { return ''; }
}

export function guardarFestival(slug) {
  try {
    if (slug) localStorage.setItem(CLAVE_FESTIVAL, slug);
    else localStorage.removeItem(CLAVE_FESTIVAL);
  } catch { /* modo privado */ }
}
...
/** Los festivales cargados, el que corre primero. */
export async function listarFestivales(codigo = leerCodigo()) {
  const { data, error } = await db(codigo)
    .from('festivales').select('*').order('fecha_inicio', { ascending: false });
  if (error) throw new Error(error.message);
  return data ?? [];
}

// Sin elección explícita, manda el que está ocurriendo hoy; si ninguno corre,
// el próximo que empiece; y si todos pasaron, el último. Abrir la app en mitad
// de un festival y que muestre otro sería absurdo.
export function escogerFestival(festivales, hoy, slugPreferido) {
  if (slugPreferido) {
    const elegido = festivales.find(f => f.slug === slugPreferido);
    if (elegido) return elegido;
  }
  const corriendo = festivales.find(f => f.fecha_inicio <= hoy && hoy <= f.fecha_fin);
  if (corriendo) return corriendo;
  const proximos = festivales.filter(f => f.fecha_inicio > hoy)
    .sort((a, b) => a.fecha_inicio.localeCompare(b.fecha_inicio));
  return proximos[0] ?? festivales[0] ?? null;
}

/** Lee todo el festival activo. Devuelve null si el código no abre nada. */
export async function cargarPanorama(codigo = leerCodigo(), slugFestival = leerFestival()) {
  const c = db(codigo);

  const festivales = await listarFestivales(codigo);
  if (!festivales.length) return null;
  const festival = escogerFestival(festivales, hoyMedellin(), slugFestival);
  ...
  // Las fotos viven en un bucket privado, así que hay que firmarlas. Se firman
  // todas de una vez: una petición por función sería una cascada de decenas.
  const delFestival = funciones.filter(f => f.festival_id === festival.id);
  const claves = [...new Set(delFestival.map(f => f.imagen_key).filter(Boolean))];
  const firmadas = {};
  if (claves.length) {
    const { data } = await c.storage.from(BUCKET).createSignedUrls(claves, 3600);
    for (const x of data ?? []) if (x.signedUrl) firmadas[x.path] = x.signedUrl;
  }

  return armarPanorama({
    festival, salas, festivales,
    funciones: delFestival.map(f => ({ ...f, imagen_url: firmadas[f.imagen_key] ?? null })),
...
/** Mete o saca una función de la agenda. */
export async function marcarAgendada(id, agendada, codigo = leerCodigo()) {
  const { error } = await db(codigo).from('funciones').update({ agendada }).eq('id', id);
  if (error) throw new Error(error.message);
}

/**
 * Triaje: me interesa / no me interesa / sin ojear.
 *
 * Se manda null para volver a "sin ojear", que no es lo mismo que un 'no': con
 * 776 funciones la mayoría no se ha mirado nunca, y poder deshacer un descarte
 * hecho de afán importa más que ahorrarse un estado.
 */
export async function marcarInteres(id, interes, codigo = leerCodigo()) {
  const { error } = await db(codigo).from('funciones')
    .update({ interes: interes ?? null }).eq('id', id);
  if (error) throw new Error(error.message);
}

/** Descartar una franja entera de un golpe: 317 lanzamientos, uno por uno, no. */
export async function marcarInteresVarias(ids, interes, codigo = leerCodigo()) {
  if (!ids.length) return;
  const { error } = await db(codigo).from('funciones')
    .update({ interes: interes ?? null }).in('id', ids);
  if (error) throw new Error(error.message);
}
