...
import {
  razonesDe, avisoCuerpo, ordenarParaOjear, contarOjeo, paraDecidir, REGLAS,
...
test('agrupar por obra junta la misma obra repetida en fechas distintas', () => {
  const fs = [
    F({ obra: 'Dark Circus Stereoptik', fecha: '2026-11-01' }),
    F({ obra: 'Dark Circus Stereoptik', fecha: '2026-11-02' }),
    F({ obra: 'Otra obra', fecha: '2026-11-01' }),
  ];
  const grupos = agruparPorObra(fs);
  assert.equal(grupos.length, 2);
  assert.equal(grupos.find(g => g[0].obra === 'Dark Circus Stereoptik').length, 2);
});

test('el interés del grupo solo es si/no cuando todas sus funciones coinciden', () => {
  assert.equal(interesDeGrupo([F({ interes: 'si' }), F({ interes: 'si' })]), 'si');
  assert.equal(interesDeGrupo([F({ interes: 'no' }), F({ interes: 'no' })]), 'no');
  assert.equal(interesDeGrupo([F({ interes: 'si' }), F({ interes: null })]), null);
  assert.equal(interesDeGrupo([F({ interes: 'si' }), F({ interes: 'no' })]), null);
  assert.equal(interesDeGrupo([F({}), F({})]), null);
});
