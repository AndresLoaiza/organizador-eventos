...
test('dos boletas que vienen del mismo PDF completan la función', () => {
  // El operador manda un solo archivo con las dos entradas cuando se compran
  // juntas. Contar archivos en vez de boletas producía la alarma falsa
  // "necesitas 2 y solo hay 1" en funciones que estaban completas.
  const conDos = [
    ...BOLETAS,
    { id: 'b7b', funcion_id: 'habit', archivo_id: 'arch-habitar', pagina: 2, hora_boleta: 1200, valor_ticket: 10900 },
  ].map(b => b.id === 'b7' ? { ...b, archivo_id: 'arch-habitar', pagina: 1 } : b);

  const a = detectarAvisos({ funciones: FUNCIONES, boletas: conDos, hoy: '2026-08-24', travel })
    .filter(x => x.tipo === 'boletas_insuficientes');
  assert.equal(a.length, 0);
});
