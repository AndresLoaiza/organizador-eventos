...
/** Minutos desde medianoche, hora de Medellín. */
export function minutosAhoraMedellin() {
...
/**
 * ¿Ya terminó esta función?
 *
 * Comparar solo la fecha no basta y el error no es teórico: el 25 de agosto, a
 * media tarde, Petra —esa misma noche a las 8— contaba como vista, se ordenaba
 * primero por ser la más reciente y quedaba de opción por defecto al registrar
 * un juicio. El resultado fue que la impresión de KRAPP, del día anterior, se
 * guardó contra una obra que todavía no había empezado.
 */
export function yaTermino(f, hoy, ahora) {
  const fecha = aISO(f.fecha);
  if (fecha < hoy) return true;
  if (fecha > hoy) return false;
  return f.hora_min + (f.duracion_min ?? 0) <= ahora;
}
...
export function armarPanorama({ festival, festivales = [], salas, funciones: filas, boletas, estados: estadosFilas, bitacora, traslados }) {
  ...
  const decisor = crearDecisor(
    ...
    slug => venues[slug]?.n ?? null,
  );

  return {
    festival, festivales, salas, funciones: porFuncion, boletas, avisos, hoy, travel, decisor,
